...
Архитектура (Feature Tokenizer + Transformer Encoder):
  ...
  - Stack: (B, 12, d_model)  [10 числовых + 1 cat + 1 CLS]
  - TransformerEncoder: attention между признаками (не между временными шагами!)
  - Выход CLS-токена → регрессионная голова → 1 число (predicted_remaining)
...
class FTTransformerModel(nn.Module):
    ...
    ) -> torch.Tensor:                # (B,) — предсказание остатка продаж
        """
        Прямой проход.

        Returns
        -------
        torch.Tensor : shape (B,) — predicted_remaining_sales (в масштабе скейлера)
        """
